...
static vibe_display_render_signature_t last_render_signature;
...
void vibe_display_init(void)
{
    ...
    vibe_display_render_signature_reset(&last_render_signature);
...
void vibe_display_show_status(const vibe_status_packet_t *packet)
{
    ...
    if (display_ready && xSemaphoreTake(display_mutex, pdMS_TO_TICKS(500)) == pdTRUE) {
        ...
        vibe_display_render_change_t render_change = vibe_display_render_change(&last_render_signature, packet);
        if (render_change != VIBE_DISPLAY_RENDER_NONE) {
            if (!preserve_animation_tick) {
                animation_tick = 0;
            }
            if (vibe_display_status_refresh_advances_animation(packet->state) &&
                !vibe_display_phase_refresh_enabled(packet->state)) {
