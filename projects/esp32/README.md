...
- `tasks[]` 非空时，底部贴边任务面板显示最多 5 条任务行；每条任务只要带有 `detail` 就会显示详情，任务标题右侧会显示 `CTX` 上下文已用百分比。
...
| `main/vibe_display_model.*` | 渲染签名、任务行格式、紧凑计数、参考迷宫坐标、本轮已吃豆子隐藏、`SCORE` / `HIGH SCORE` / `LEVEL` 规则、角色数量和角色嘴型。 |
