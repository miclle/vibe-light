...
- 目标板横放时会通过 QMI8658 加速度方向切换到 820 x 320 横屏画面；竖立或 IMU 不可用时保持当前竖屏画面。横屏画面不改变 BLE 协议，顶部展示 `VIBE LIGHT`、Codex `5H` / `7D` 和 `RESET` 信息，中间展示全宽截图样式吃豆人迷宫，底部展示 `SCORE`、`HIGH SCORE`、`LEVEL`、`CODEX LIVE` 和 `FW ...`。
