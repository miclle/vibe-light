...
- `tasks[]` 非空时，底部贴边任务面板显示最多 5 条任务行；每条任务只要带有 `detail` 就会显示详情，任务标题右侧显示 `RUN 03:12`、`WAIT 01:08`、`2m ago`、`CTX xx%` 或 `CTX 4.2K/12K`。活跃任务会在运行 / 等待时长和 `CTX` 之间低频轮播，高上下文占用时提高 `CTX` 出现频率；缺少 `updatedAt` 或时间无效时回退显示 `CTX`。每条任务左侧用内缩小色块标记状态，避免贴屏幕边缘形成竖线。页脚短状态左对齐显示来源和链路状态，例如 `CODEX LIVE`；旧协议包显示 `CODEX LEGACY`，不直接暴露 `v1` / `v2` 这类内部协议版本。页脚使用中性灰并保留 12px 底部余量；活跃/等待/错误计数只显示在迷宫里的 `ACTIVE` / `WAIT` / `ERR` 计数区，避免重复。
