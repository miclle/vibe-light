...
- `tasks[]` 非空时，底部贴边任务面板显示最多 5 条任务行；每条任务只要带有 `detail` 就会显示详情，任务标题右侧显示 `RUN 03:12`、`WAIT 01:08`、`2m ago`、`CTX xx%` 或 `CTX 4.2K/12K`。活跃任务会在运行 / 等待时长和 `CTX` 之间低频轮播，高上下文占用时提高 `CTX` 出现频率；缺少 `updatedAt` 或时间无效时回退显示 `CTX`。页脚短状态左对齐显示来源和协议版本，例如 `CODEX V2`，并保留 12px 底部余量；活跃/等待/错误计数只显示在迷宫里的 `ACTIVE` / `WAIT` / `ERR` 计数区，避免重复。
- `tasks[]` 为空时显示 `NO ACTIVE TASKS`；如果整体 `detail` 带有 `LAST OK ...` 或 `LAST ERR ...` 这类最近结果摘要，会优先显示该摘要，否则回退到整体状态标题。
