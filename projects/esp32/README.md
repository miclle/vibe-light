...
- `tasks[]` 非空时，底部贴边任务面板显示最多 5 条任务行；每条任务只要带有 `detail` 就会显示详情，任务标题右侧显示 `RUN 03:12`、`WAIT 01:08`、`2m ago`、`CTX xx%` 或 `CTX 4.2K/12K`。活跃任务会在运行 / 等待时长和 `CTX` 之间低频轮播，高上下文占用时提高 `CTX` 出现频率；缺少 `updatedAt` 或时间无效时回退显示 `CTX`。
