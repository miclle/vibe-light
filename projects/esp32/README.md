...
- 目标板横放时会通过 QMI8658 加速度方向切换到 820 x 320 横屏画面；竖立或 IMU 不可用时保持当前竖屏画面。横屏画面不改变 BLE 协议，整屏展示由 `docs/Pac-Man-landscape.png` 生成的截图样式吃豆人画面，不再额外叠加顶部或底部状态栏；`busy` 时计分、关卡和 `HIGH SCORE` 推进仍沿用竖屏同一套显示模型规则。
...
- 竖屏动画由 `esp_timer` 周期 tick 驱动，周期为 240ms；状态变化时补齐 RGB driver 三 framebuffer，稳定动画 tick 只重绘非当前 framebuffer 的 320px 迷宫舞台，再整屏切换显示，避免在 RGB LCD 扫描当前 framebuffer 时改写画面，也避免每帧重写整屏。横屏 `busy` 仍显示整屏 RLE 画面，不启用竖屏吃豆人刷新循环。
...
- 切换到非竖屏 `busy` 后停止 timer；`waiting` 不绘制吃豆动画，只随新的状态包重绘等待任务和尾标。

## 已知显示问题

- 当前实机仍观察到轻微屏幕闪烁，而且在没有吃豆人动画时也会出现。
- 这类现象不应只按竖屏动画刷新问题处理；下一步优先排查 ST7701 RGB LCD 初始化参数、PCLK / porch / bounce buffer、PSRAM 带宽、VSYNC / DMA 生命周期、背光 PWM 频率和硬件供电稳定性。
- 最近烧录确认版本为 `v0.1.2-25-ge420d99`，该版本已使用 RGB driver 三 framebuffer 轮转，并确认 macOS compact BLE 包会保留 `usage`，屏幕可恢复显示 `CODEX: 5H / 7D`。
...
| `main/vibe_display.*` | ST7701 LCD 初始化、PSRAM framebuffer、背光 PWM、横竖屏调度、最终 panel flush 和动画 timer。 |
| `main/vibe_display_portrait.*` | 竖屏布局、任务列表、顶部 / 页脚 / 计分文本和竖屏吃豆人动画。 |
| `main/vibe_display_landscape.*` | 横屏 RLE 画面渲染和横屏 framebuffer 到物理竖屏 framebuffer 的旋转。 |
| `main/vibe_orientation.*` | QMI8658 方向读取和横竖屏切换；失败时保持竖屏。 |
