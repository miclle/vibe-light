...
- 顶部色条居中显示 `VIBE LIGHT`，活跃任务界面颜色来自整体 `state`；空任务界面使用更暗的安静头部，避免 `NO ACTIVE TASKS` 时整块顶部过亮。如果状态包带有 Codex 用量，会同时显示 `CODEX: 5H ... 7D ...` 剩余百分比；低余量且带 reset 时间时，百分比继续常驻显示，并在下方额外显示 `5H RESET 45m` 这类短提示。
