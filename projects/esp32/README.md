...
- 竖屏动画由 `esp_timer` 周期 tick 驱动，周期为 240ms；状态变化时补齐 RGB driver 双 framebuffer，稳定动画 tick 只重绘非当前 framebuffer 的 320px 迷宫舞台，再整屏切换显示，避免在 RGB LCD 扫描当前 framebuffer 时改写画面，也避免每帧重写整屏。横屏 `busy` 仍显示整屏 RLE 画面，不启用竖屏吃豆人刷新循环。
