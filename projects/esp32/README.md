...
屏幕驱动当前已接入 Waveshare `ESP32-S3-LCD-3.16` 的 ST7701 RGB LCD 初始化、主动低电平背光 PWM 和基础 RGB565 绘制。`vibe_display_show_status` 会把状态写入串口日志，并在 LCD 上渲染顶部状态色、参考迷宫、任务计数和任务列表。
...
| `main/vibe_display_model.*` | 渲染签名、任务行格式、紧凑计数、参考迷宫坐标、豆子恢复、角色数量和角色嘴型。 |
