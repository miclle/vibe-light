...
- 顶部色条显示 `VIBE LIGHT`，颜色来自整体 `state`。
- 中间面板显示 Codex 吃豆人迷宫任务舞台，不再重复显示 `source` 或 `x running` 调试文字。
- `tasks[]` 非空时，底部贴边任务面板显示紧凑的 `A`、`W`、`E` 计数和最多 5 条任务行。
- `tasks[]` 为空时显示 `NO ACTIVE TASKS` 和单条 detail。
- 未知状态降级为 `idle`；缺少必需 `state` 或非法 JSON 时拒绝包，并保持旧显示状态。
- 重复包通过 `vibe_display_model` 的渲染签名去重，减少静态界面重复重绘。

`busy` 状态会启动 Codex 吃豆人中部迷宫动画：

- 动画由 `esp_timer` 周期 tick 驱动，周期为 180ms。
- 任务列表里有几个任务，就显示几个主角；没有任务列表时回退到 `activeCount`。
- 主角沿中间面板里的简化迷宫路径移动，豆子避开主角当前位置，嘴型随 tick 开合。
- 动画速度由 `activeCount` 调节：1 个活跃任务正常速度，2 个及以上加速，5 个及以上进一步加速。
...
| `main/vibe_display_model.*` | 渲染签名、任务行格式、动画路径、动画速度和角色嘴型。 |
