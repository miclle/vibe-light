...
- 顶部色条居中显示 `VIBE LIGHT`，颜色来自整体 `state`；如果状态包带有 Codex 用量，会同时显示 `CODEX: 5H ... 7D ...` 剩余百分比；低余量且带 reset 时间时，会优先显示 `CODEX: 5H RESET 45m` 这类提示。
- 中间面板显示 Codex 吃豆人参考迷宫任务舞台，不再重复显示 `source` 或 `x running` 调试文字；顶部的 `SCORE` 来自动画已吃豆子，`HIGH SCORE` 保存在 ESP32 NVS 中，`LEVEL` 随完整吃豆轮数递增。
- `tasks[]` 非空时，底部贴边任务面板显示最多 5 条任务行；每条任务只要带有 `detail` 就会显示详情，任务标题右侧优先显示 `RUN 03:12`、`WAIT 01:08` 或 `2m ago` 这类任务时长 / 新鲜度；缺少 `updatedAt` 或时间无效时回退显示 `CTX` 上下文已用百分比。
