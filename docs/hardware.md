...
- 通过健康状态特征回传运行时长、连接状态、最近状态、heap 余量和渲染 tick。
...
4. **验证健康状态回读**
   - 通过健康状态 characteristic 回传协议版本、设备名、运行时长、连接状态、最近状态、heap 余量和渲染 tick。
...
当前项目记录已经覆盖发现、连接、`v: 2` 写入、基础屏幕确认和 `61a603d` 的 5 分钟短稳定性采样；后续验收应继续补充 30-60 分钟长时间运行、重复连接、低余量 reset 提示、等待态尾标轮播、页脚 / 色块边缘修正、macOS 硬件页健康读数和错误包恢复等场景。
