...
每个普通直插 LED 必须使用独立限流电阻，默认使用 330Ω；固件按主动高电平驱动，三个状态通道独立判断，多个条件同时成立时对应 LED 会同步慢闪。
...
当前项目记录已经覆盖发现、连接、`v: 2` 写入、基础屏幕确认、`61a603d` 的 5 分钟短稳定性采样，`82d2180` 的健康诊断字段实机读取、肉眼屏幕复核和长时间稳定性观察，以及 `524c46d` / `939583d` 的 `CTX` 颜色分级实机复核；后续验收可继续补充重复连接、低余量 reset 提示、等待态尾标轮播和错误包恢复等场景。
