...
- `busy` 状态下随新的状态包推进 Codex 吃豆人动画，避免在 RGB LCD 上高频整屏刷新造成可见扫动；横屏 `busy` 不启用竖屏吃豆人刷新循环，只渲染横屏 RLE 画面。`waiting` 状态不推进吃豆动画，只随状态包重绘等待任务和尾标。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、本轮已吃豆子隐藏、`SCORE`、`HIGH SCORE` 和状态切换；`busy` 动画随状态包到达推进，横屏 `busy` 保持静态 RLE 画面。`HIGH SCORE` 在运行中内存更新，每新增约 1000 分或离开 `busy` 状态时写入 NVS，避免每次显示刷新都刷写 flash。
