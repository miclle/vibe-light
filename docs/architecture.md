...
当前 BLE 状态包使用 `v: 2`，除了整体 `source` / `state` / `detail` 外，还包含任务计数和最多 5 个任务行。ESP32-S3 屏幕顶部显示聚合状态，中间保留给参考迷宫动画舞台，底部贴边区域显示任务计数和列表。固件仍兼容旧的 `v: 1` 单状态包；旧包没有 `tasks[]` 时，屏幕会回退为空任务面板和单条状态说明。macOS 端如果发现完整 `v: 2` JSON 超过 BLE 单次写入长度，会自动降级为 `v: 1` 包，保证旧链路仍可写入。
...
| `tasks` | array | 否 | ESP32-S3 屏幕列表行，最多发送 5 条；每条包含 `title`、`source`、`state` 和可选 `detail`。任务标题限制为 32 个 UTF-8 字节，任务详情限制为 40 个 UTF-8 字节。 |
...
- 屏幕顶部用状态色显示 `VIBE LIGHT`。
- 中间区域显示 320px 参考迷宫任务舞台，不再重复显示来源或 `x running` 等调试文字；底部贴边区域显示紧凑任务计数和最多 5 条任务摘要。
- 重复状态包通过显示签名去重；只有状态、计数或任务文本变化时才重绘静态内容。
