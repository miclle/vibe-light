...
- 中间区域显示 Codex 吃豆人迷宫任务舞台，不再重复显示来源或 `x running` 等调试文字；底部贴边区域显示紧凑任务计数和最多 5 条任务摘要。
- 重复状态包通过显示签名去重；只有状态、计数或任务文本变化时才重绘静态内容。
- `busy` 状态下由 `esp_timer` 周期 tick 刷新 Codex 吃豆人中部舞台动画，避免阻塞 BLE 回调。
...
| 状态 | 吃豆人动画表现 |
| --- | --- |
| `busy` | 每个任务对应一个主角，在中间迷宫路径上循环吃豆子；任务越多可适度提高速度或增加豆子亮度。 |
