...
- 中间区域显示 320px 参考迷宫任务舞台，不再重复显示来源或 `x running` 等调试文字；迷宫顶部显示随已吃豆子累计增长的 `SCORE`、保存在 ESP32 NVS 中的 `HIGH SCORE` 和随完整吃豆轮数递增的 `LEVEL`；底部贴边区域显示紧凑任务计数、最多 5 条任务摘要和任务右侧尾标。活跃任务尾标在运行 / 等待时长和 `CTX` 上下文已用百分比之间低频轮播，高上下文占用时提高 `CTX` 出现频率；完成或失败任务显示新鲜度，缺少时间时回退为 `CTX`。
