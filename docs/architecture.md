...
- 竖屏 `busy` 状态下使用 240ms 非阻塞 timer 推进 Codex 吃豆人动画；状态变化时补齐 RGB driver 三 framebuffer，稳定动画 tick 只重绘非当前 framebuffer 的 320px 迷宫舞台，再整屏切换显示，避免在 RGB LCD 扫描当前 framebuffer 时改写画面，也避免每帧重写整屏；横屏 `busy` 不启用竖屏吃豆人刷新循环，只渲染横屏 RLE 画面。`waiting` 状态不推进吃豆动画，只随状态包重绘等待任务和尾标。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、本轮已吃豆子隐藏、`SCORE`、`HIGH SCORE` 和状态切换；竖屏 `busy` 动画由本地 240ms timer 推进，并通过 RGB driver 三 framebuffer 切换提交，常规动画帧只重绘迷宫舞台，横屏 `busy` 保持静态 RLE 画面。`HIGH SCORE` 在运行中内存更新，每新增约 1000 分或离开 `busy` 状态时写入 NVS，避免每次显示刷新都刷写 flash。
