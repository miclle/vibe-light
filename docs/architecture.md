...
- `busy` 状态下不再使用 240ms 定时整屏刷新；固件在收到新的状态包时推进 Codex 吃豆人动画，避免 RGB LCD 持续闪屏。`waiting` 状态不推进吃豆动画，只随状态包重绘等待任务和尾标。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、本轮已吃豆子隐藏、`SCORE`、`HIGH SCORE` 和状态切换；当前 `busy` 动画随状态包到达推进。`HIGH SCORE` 在运行中内存更新，每新增约 1000 分或离开 `busy` 状态时写入 NVS，避免每次显示刷新都刷写 flash。
