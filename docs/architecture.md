...
显示模型相关逻辑位于 `vibe_display_model.*`，包括 FNV-1a 渲染签名、参考迷宫路径、213 个豆子、4 个能量豆、本轮已吃豆子隐藏、累计 `SCORE` 计分、`LEVEL` 轮数计算、所有豆子被吃完后一次性视觉重置、最多 5 个错相角色和角色嘴型方向。显示文案格式位于 `vibe_display_format.c`，包括任务行、用量 reset 提示、页脚和固件版本短文本。`HIGH SCORE` 的 NVS 持久化位于 `vibe_display_score.*`。硬件绘制位于 `vibe_display.c`，负责 framebuffer、LCD panel、背光、状态色和具体图形绘制。`projects/esp32/tools/render_maze_preview.py` 可在没有硬件时生成迷宫和全屏 PNG 预览。
