...
- 竖屏 `busy` 状态下使用 240ms 非阻塞 timer 推进 Codex 吃豆人动画；固件使用应用侧 PSRAM framebuffer 绘制完整竖屏画面，再交给 RGB driver 的双 framebuffer 路径提交，避免直接轮转 driver 内部 framebuffer。`waiting` 状态不推进吃豆动画，只随状态包重绘等待任务和尾标。当前取舍为竖屏稳定优先，横屏自动切换和 driver 内部 framebuffer 轮转暂停启用。

显示模型相关逻辑位于 `vibe_display_model.*`，包括 FNV-1a 渲染签名、参考迷宫路径、横屏布局矩形、横竖屏 hysteresis、213 个豆子、4 个能量豆、本轮已吃豆子隐藏、累计 `SCORE` 计分、`LEVEL` 轮数计算、所有豆子被吃完后一次性视觉重置、最多 5 个错相角色和角色嘴型方向。显示文案格式位于 `vibe_display_format.c`，包括任务行、用量 reset 提示、页脚和固件版本短文本。横屏整屏截图样式 RLE 数据位于 `vibe_landscape_maze_data.*`；QMI8658 方向读取位于 `vibe_orientation.*`，但当前实机启动路径不启用自动横竖屏切换。`HIGH SCORE` 的 NVS 持久化位于 `vibe_display_score.*`。`vibe_display.c` 负责 framebuffer、LCD panel、背光、竖屏最终 panel flush 和动画 timer；`vibe_display_portrait.*` 拥有竖屏布局、任务列表和竖屏吃豆人动画；`vibe_display_landscape.*` 保留横屏 RLE 画面和旋转写回物理 framebuffer 的 host-side 预览能力。`projects/esp32/tools/render_maze_preview.py` 可在没有硬件时生成迷宫、竖屏全屏和横屏全屏 PNG 预览。
...
| 没有 Agent 条件，包括空闲、离线或未知用量 | 绿 5 秒、黄 2 秒、红 5 秒、黄 2 秒循环 | 安静的空闲或断开界面。 |

三色灯三路 Agent 条件独立求值，并统一采用 1 秒慢闪周期（亮 500 ms、灭 500 ms）。例如同时存在错误任务、执行中任务和最近完成任务时，红、黄、绿三灯会同步闪烁。任一 Agent 条件存在时，这组灯效完整覆盖固件本地交通灯；交通灯从启动自检结束时建立独立的单调时间轴，Agent 条件全部结束后恢复到当时相位，不重置循环。BLE 断开时没有可用 Agent 条件，因此继续显示交通灯。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、本轮已吃豆子隐藏、`SCORE`、`HIGH SCORE` 和状态切换；当前版本用 240ms 竖屏本地 timer 推进 `busy` 动画，并锁定竖屏运行时以避开 RGB driver 内部 framebuffer 轮转导致的实机微闪。`HIGH SCORE` 在运行中只更新内存，离开 `busy` 状态时写入 NVS，避免运行中的 flash 写入影响显示刷新。
...
- **状态机简单**：固件接受 macOS 下发的聚合状态，不自行推断任务生命周期。LCD 连接时显示 `idle / desktop connected`，断开时显示 `offline / desktop disconnected`；LED 在没有 Agent 条件时只运行固件本地交通灯循环。
