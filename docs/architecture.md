...
显示模型相关逻辑位于 `vibe_display_model.*`，包括任务行格式、FNV-1a 渲染签名、参考迷宫路径、213 个豆子、4 个能量豆、本轮已吃豆子隐藏并在所有豆子被吃完后一次性重置、最多 5 个错相角色和角色嘴型方向。硬件绘制位于 `vibe_display.c`，负责 framebuffer、LCD panel、背光、状态色和具体图形绘制。`projects/esp32/tools/render_maze_preview.py` 可在没有硬件时生成迷宫和全屏 PNG 预览。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、本轮已吃豆子隐藏和状态切换；当前动画 tick 周期为 240ms。
