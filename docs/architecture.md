...
当前 BLE 状态包使用 `v: 2`，除了整体 `source` / `state` / `detail` 外，还包含任务计数、Codex 用量摘要和最多 5 个任务行。ESP32-S3 屏幕顶部显示聚合状态和 Codex 5h / 7d 剩余百分比，中间保留给参考迷宫动画舞台，底部贴边区域显示任务计数、列表和任务新鲜度 / 运行时长。固件仍兼容旧的 `v: 1` 单状态包；旧包没有 `tasks[]` 时，屏幕会回退为空任务面板和单条状态说明。macOS 端如果发现完整 `v: 2` JSON 超过 BLE 单次写入长度，会自动降级为 `v: 1` 包，保证旧链路仍可写入。
