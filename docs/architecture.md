...
- 横放目标板时，固件会通过 QMI8658 加速度读数切换到 820 x 320 横屏 framebuffer，再旋转写入现有 320 x 820 LCD；竖立或 IMU 不可用时保持竖屏。横屏不改变 `StatusPacket` 或 BLE 协议，整屏显示由 `docs/Pac-Man-landscape.png` 生成的截图样式吃豆人 RLE 画面，不额外叠加顶部或底部状态栏；`busy` 时计分、关卡和 `HIGH SCORE` 推进仍沿用竖屏同一套显示模型规则。
- 重复状态包通过显示签名去重；只有状态、计数、用量或任务文本变化时才重绘静态内容。
- 竖屏 `busy` 状态下使用 240ms 非阻塞 timer 推进 Codex 吃豆人动画；状态变化时补齐 RGB driver 三 framebuffer，稳定动画 tick 只重绘非当前 framebuffer 的 320px 迷宫舞台，再整屏切换显示，降低在 RGB LCD 扫描当前 framebuffer 时改写画面的风险，也避免每帧重写整屏；横屏 `busy` 不启用竖屏吃豆人刷新循环，只渲染横屏 RLE 画面。`waiting` 状态不推进吃豆动画，只随状态包重绘等待任务和尾标。当前实机仍观察到轻微屏幕闪烁，且无动画时也会出现，后续应继续从 RGB LCD 时序、VSYNC / DMA 生命周期、背光 PWM 或供电稳定性排查。

显示模型相关逻辑位于 `vibe_display_model.*`，包括 FNV-1a 渲染签名、参考迷宫路径、横屏布局矩形、横竖屏 hysteresis、213 个豆子、4 个能量豆、本轮已吃豆子隐藏、累计 `SCORE` 计分、`LEVEL` 轮数计算、所有豆子被吃完后一次性视觉重置、最多 5 个错相角色和角色嘴型方向。显示文案格式位于 `vibe_display_format.c`，包括任务行、用量 reset 提示、页脚和固件版本短文本。横屏整屏截图样式 RLE 数据位于 `vibe_landscape_maze_data.*`。QMI8658 方向读取位于 `vibe_orientation.*`，失败时保持竖屏。`HIGH SCORE` 的 NVS 持久化位于 `vibe_display_score.*`。`vibe_display.c` 负责 framebuffer、LCD panel、背光、横竖屏调度和最终 panel flush；`vibe_display_portrait.*` 拥有竖屏布局、任务列表和竖屏吃豆人动画；`vibe_display_landscape.*` 拥有横屏 RLE 画面和旋转写回物理 framebuffer 的逻辑。`projects/esp32/tools/render_maze_preview.py` 可在没有硬件时生成迷宫、竖屏全屏和横屏全屏 PNG 预览。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、本轮已吃豆子隐藏、`SCORE`、`HIGH SCORE` 和状态切换；竖屏 `busy` 动画由本地 240ms timer 推进，并通过 RGB driver 三 framebuffer 切换提交，常规动画帧只重绘迷宫舞台，横屏 `busy` 保持静态 RLE 画面。当前仍有轻微屏幕闪烁待底层排查。`HIGH SCORE` 在运行中内存更新，每新增约 1000 分或离开 `busy` 状态时写入 NVS，避免每次显示刷新都刷写 flash。
...
灯珠数量、多灯动画、外壳结构、电源管理和配网能力不属于当前架构层核心边界。横屏模式已经作为 ESP32 本地显示适配落地，但仍不属于 BLE 协议语义；后续如果引入 LED 或其他屏幕，应继续复用同一个 `StatusPacket` 语义，由硬件层内部适配输出设备。
