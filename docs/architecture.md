...
- 横放目标板时，固件会通过 QMI8658 加速度读数切换到 820 x 320 横屏 framebuffer，再旋转写入现有 320 x 820 LCD；竖立或 IMU 不可用时保持竖屏。横屏不改变 `StatusPacket` 或 BLE 协议，顶部显示 `VIBE LIGHT`、Codex `5H` / `7D` 和 `RESET` 信息，中间显示全宽截图样式吃豆人 RLE 迷宫，底部显示 `SCORE`、`HIGH SCORE`、`LEVEL`、`CODEX LIVE` 和 `FW ...`。
