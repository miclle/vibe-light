...
当前 BLE 状态包使用 `v: 2`，除了整体 `source` / `state` / `detail` 外，还包含任务计数和最多 5 个任务行。ESP32-S3 屏幕顶部显示聚合状态，中间显示多任务列表。固件仍兼容旧的 `v: 1` 单状态包；旧包没有 `tasks[]` 时，屏幕会回退为单条 detail 展示。macOS 端如果发现完整 `v: 2` JSON 超过 BLE 单次写入长度，会自动降级为 `v: 1` 包，保证旧链路仍可写入。
...
| Display Model | 计算渲染签名、任务行文案、动画路径、动画速度和角色嘴型，让核心显示规则可以在 host-side 测试里验证。 |
| Display Controller | 根据聚合状态驱动 ST7701 RGB LCD：顶部显示总体状态，v2 包有 `tasks[]` 时显示任务列表，否则显示单条 detail；`busy` 时启动非阻塞动画 timer。 |
...
- 屏幕顶部用状态色显示 `VIBE LIGHT` 和聚合状态。
- 中间区域显示来源、状态标题、detail、任务计数和最多 5 条任务摘要。
- 重复状态包通过显示签名去重；只有状态、计数或任务文本变化时才重绘静态内容。
- `busy` 状态下由 `esp_timer` 周期 tick 刷新 Codex 吃豆人边缘动画，避免阻塞 BLE 回调。

显示模型相关逻辑位于 `vibe_display_model.*`，包括任务行格式、FNV-1a 渲染签名、动画路径、活跃任务数驱动的速度变化和角色嘴型方向。硬件绘制位于 `vibe_display.c`，负责 framebuffer、LCD panel、背光和具体图形绘制。
...
当前 IPS TFT 动画方向是把 `busy` 状态表现成“Codex 吃豆人”：当 Codex 有任务在执行时，屏幕四周出现一圈豆子，主角沿屏幕边缘循环移动并不断吃豆子；只要聚合状态仍是 `busy`，动画就一圈一圈持续运行，直到任务结束后切换到 `success`、`waiting` 或 `error` 的对应表现。
...
- ESP32-S3 固件在 Display Controller 内根据 `state` 自行维护动画 tick、主角位置、豆子重置和状态切换；当前动画 tick 周期为 180ms。
- `activeCount`、`waitingCount`、`errorCount` 可以作为动画参数，但不应改变协议语义。
- 当前 `activeCount >= 2` 时动画路径步进加快，`activeCount >= 5` 时进一步提速。
