...
| Task Tracker | 根据 task id、workspace 或 source 聚合同步到达的多任务事件，生成整体状态和最多 5 条屏幕任务摘要。 |
...
`v: 2` 状态包表示 macOS 聚合后的整体显示状态和屏幕列表，不表示单个 hook 事件的原始 payload。Codex 用量来自本地 transcript 的最新 `token_count` 事件，`tasks[]` 是展示用摘要，不是硬件侧会话生命周期模型。
...
- 中间区域显示 320px 参考迷宫任务舞台，不再重复显示来源或 `x running` 等调试文字；底部贴边区域显示紧凑任务计数、最多 5 条任务摘要和任务右侧上下文已用百分比。
