...
工程侧当前可用路径已经落到 GitHub Actions 生成的 Developer ID notarized pre-release。`v0.1.0-beta.1` 已作为第一版公开 beta pre-release 发布并完成下载包验证；剩余事项主要是观察 beta 反馈、准备稳定版 release gate、正式发布法律 / 合规确认、发布自动化维护和失败恢复回归。Python runtime、签名 / notarization 和 GPL source gate 不再是当前阻塞点。
...
1. 发布签名、notarization 和烧录前芯片确认已完成验证
   ...
   - 会启动崩溃的 draft `v2026.06.14-dev-08c645b` 已删除且无残留 tag；当前远端保留推荐 beta tag `v0.1.0-beta.1` 和历史回退 tag `v2026.06.14-dev-d5dd54a`。
   ...
   - 2026-06-14 已通过 GitHub Actions 发布 `v0.1.0-beta.1` pre-release：workflow run `27484764574` 完整通过，release URL 为 `https://github.com/miclle/vibe-light/releases/tag/v0.1.0-beta.1`，tag 指向 `f96c3cf4d83649fd709f3248d83256e9378b0364`。
   - `v0.1.0-beta.1` 下载 zip SHA-256 为 `062c35035451f0c9efb6ce4331988adef414d2eda605eedcd10ea1160b0e6bb3`，checklist asset SHA-256 为 `12b70e32a36938ae88fa61d466a8427fb3f8721c58844713460ec167c416d3ed`；两者均与 GitHub asset digest 一致。
   - `v0.1.0-beta.1` 下载 zip 用 `ditto -x -k` 解压后通过 `xcrun stapler validate`、`syspolicy_check distribution`、`codesign --verify --deep --strict` 和 strict helper `--help`；下载形态 app 可启动。
   - 同一 release 包内 GPL/source gate 复核通过：`THIRD_PARTY_NOTICES.md`、`OPEN_SOURCE_NOTICES.md`、`SOURCE_OFFER.md`、`pyserial-3.5.dist-info/LICENSE.txt` 和 `sources/esptool-4.11.0.tar.gz` 均存在，`SOURCE_OFFER.md` 包含 `any third party` 和实际分发成本 wording，`esptool` 源码包 SHA-256 为 `496571e4f6e36f7dc9a730dd485c4a9d522c9e7d6bb90ea2fec0a049275fbfad`。
   - 同一 release 包内 strict helper 已对 `/dev/cu.usbmodem1101` 完成 `chip_id` 和完整 `write_flash`，识别 `ESP32-S3 (QFN56)`、BLE、8MB PSRAM 和 MAC `1c:db:d4:7b:3f:cc`，bootloader、partition table 和 app 三段均 `Hash of data verified`；串口启动日志确认 `LCD initialized`、`advertising as VibeLight-S3`、desktop connected 和状态包显示。
...
5. 发布自动化已有 checklist 和 GitHub pre-release 入口
   ...
   - 当前推荐 pre-release `v0.1.0-beta.1` 已发布并完成下载包实机验证；旧 dev pre-release `v2026.06.14-dev-d5dd54a` 保留作历史回退。
...
6. 继续处理当前剩余项：观察 `v0.1.0-beta.1` beta 反馈、准备稳定版 release gate、跟进 GitHub Actions runtime 升级、维护 GPL/license 材料和失败恢复测试。
