...
- 任务行右侧会展示新鲜度、运行时长和上下文用量，例如 `RUN 03:12`、`WAIT 01:08`、`2m ago`、`CTX xx%` 或 `CTX 4.2K/12K`；活跃任务会在运行时长和 `CTX` 之间低频轮播，80% 及以上高上下文占用时提高 `CTX` 出现频率。
...
3. **优化上屏摘要规则**
   - 当前工具动作、等待态审批文案、任务新鲜度 / 运行时长 / `CTX`、token 摘要、低余量 reset 提示、footer 和 `LAST OK` / `LAST ERR` 都已实现基础展示；常见测试、构建、烧录、搜索、串口观察和 app 控制命令会压缩为短动作，80% 及以上高上下文占用会提高 `CTX` 出现频率，最近结果会随任务 stale 窗口过期，避免旧结果长期占屏。
