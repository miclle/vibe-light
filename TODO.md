...
3. **优化上屏摘要规则**
   - 当前工具动作、等待态审批文案、任务新鲜度 / 运行时长 / `CTX`、token 摘要、低余量 reset 提示、footer 和 `LAST OK` / `LAST ERR` 都已实现基础展示；常见测试、构建、烧录、搜索、串口观察和 app 控制命令会压缩为短动作，最近结果会随任务 stale 窗口过期，避免旧结果长期占屏。
   - 后续可继续基于实机可读性微调高上下文占用提示阈值、轮播节奏和最近结果摘要来源。
...
1. **先小幅打磨上屏摘要**
   - 如果长时间观察稳定，可以根据实际可读性继续微调 `CTX` 轮播节奏和最近结果摘要来源。
