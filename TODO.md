...
- 固件版本 `82d2180` 已在目标板完成烧录、串口启动、BLE 连接、健康特征实机读取和肉眼屏幕复核：LCD 初始化、BLE 广播、Central 连接、连续 `v: 2` 状态写入、token 摘要、页脚、底部余量、任务色块内缩、无边缘蓝线和 macOS 硬件页健康读数均正常；坏状态包后会回传 `lastParseError:"invalid JSON"`。
...
1. **做一轮长时间运行稳定性观察**
   - `82d2180` 已完成烧录、串口启动、BLE 连接、连续 `v: 2` 写入、健康特征读取和肉眼屏幕复核。
   - 仍建议做 30-60 分钟观察，确认 PSRAM framebuffer、ST7701 刷新、BLE 重连、动画 timer、主动低电平背光 PWM 和健康特征读取是否稳定。
...
1. **先做 30-60 分钟稳定性观察**
   - 实机视觉和健康读数已经确认正常，下一步可以专注观察长时间运行。
   - 重点看动画是否卡住、BLE 是否断连后恢复、heap 低水位是否持续下降、footer 和任务尾标是否有残影或遮挡。

2. **然后小幅打磨上屏摘要**
   - 如果长时间观察稳定，可以根据实际可读性微调工具动作摘要、`CTX` 轮播节奏和最近结果保留时长。
