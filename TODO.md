...
- 屏幕页脚会在左下方显示短状态，例如 `CODEX V2`，保留来源和协议版本；活跃/等待/错误计数只保留在迷宫里的 `ACTIVE` / `WAIT` / `ERR` 计数区，避免重复。页脚与底边保留 12px 余量，避免贴到屏幕最底部。
...
4. **评估可追加上屏信息**
  ...
  - 页脚短状态已左对齐显示来源和协议版本，例如 `CODEX V2`，并与底边保留 12px 余量；任务计数由迷宫里的 `ACTIVE` / `WAIT` / `ERR` 计数区承载。
