...
- 当前 HEAD `090391d` 已在目标板完成串口 / BLE 链路实机复核：固件启动、LCD 初始化、BLE 广播、Central 连接和 `v: 2` 状态写入均正常。token 摘要、页脚位置、页脚去重、协议版本隐藏和边缘色块修正仍需要下一次肉眼看屏复核。
...
2. **完善硬件实机诊断**
   - 下一次实机验证优先肉眼复核 token 摘要、`CODEX LIVE` / `CODEX LEGACY` 页脚、12px 底部余量、任务色块内缩和无边缘蓝线。
   - 继续记录后续烧录端口、ESP-IDF 版本和实机验证结果。
   - 已确认当前固件可在目标板上启动 ST7701 LCD、BLE 广播和 PSRAM framebuffer；后续仍需在更多运行时长下观察稳定性。
   - 健康状态包已经扩展到 heap 和渲染 tick；后续可继续补背光状态或最近解析错误。
...
4. **评估可追加上屏信息**
   ...
   - 诊断模式可显示设备本地健康信息，例如运行时长、BLE 状态、heap、渲染 tick、最近解析错误或背光状态；默认主屏不常驻，避免变成工程调试面板。
