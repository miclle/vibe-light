...
- 显示模型已从“简单任务列表”推进到 320px 竖屏参考迷宫舞台、横屏全宽截图样式吃豆人 RLE 迷宫、213 个豆子、4 个能量豆、最多 5 个错相主角、底部贴边任务面板、任务时长 / 新鲜度尾标和渲染签名去重。
...
1. **横屏 layout mode 需要实机复核**
   ...
   - 横屏原型不改 BLE 协议，使用顶部 `VIBE LIGHT` / Codex `5H` / `7D` / `RESET` 信息条，中间全宽截图样式吃豆人迷宫，底部 `SCORE` / `HIGH SCORE` / `LEVEL` / `CODEX LIVE` / `FW ...` 信息条。
   - 下一步需要肉眼确认横放方向、旋转方向、全宽截图样式迷宫清晰度和 QMI8658 读数稳定性。
