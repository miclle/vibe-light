...
3. **优化上屏摘要规则**
   - 当前工具动作、等待态审批文案、任务新鲜度 / 运行时长 / `CTX`、token 摘要、低余量 reset 提示、footer 和 `LAST OK` / `LAST ERR` 都已实现基础展示；常见测试、构建、烧录、搜索、串口观察和 app 控制命令会压缩为短动作，80% 及以上高上下文占用会提高 `CTX` 出现频率并用黄色提示，90% 及以上用红色提示，最近结果会优先使用已压缩任务详情并随任务 stale 窗口过期，避免旧结果长期占屏。
