...
- 任务行右侧会展示新鲜度、运行时长和上下文用量，例如 `RUN 03:12`、`WAIT 01:08`、`2m ago`、`CTX xx%` 或 `CTX 4.2K/12K`；活跃任务会在运行时长和 `CTX` 之间低频轮播，80% 及以上高上下文占用时提高 `CTX` 出现频率并用黄色显示，90% 及以上用红色显示。
...
1. **复核硬件诊断字段**
   ...
   - macOS 硬件页已肉眼确认能展示健康读数；后续如调整硬件页，再同步验证背光状态和最近解析错误。
...
3. **优化上屏摘要规则**
   - 当前工具动作、等待态审批文案、任务新鲜度 / 运行时长 / `CTX`、token 摘要、低余量 reset 提示、footer 和 `LAST OK` / `LAST ERR` 都已实现基础展示；常见测试、构建、烧录、搜索、串口观察和 app 控制命令会压缩为短动作，80% 及以上高上下文占用会提高 `CTX` 出现频率并用黄色提示，90% 及以上用红色提示，最近结果会随任务 stale 窗口过期，避免旧结果长期占屏。
   - 后续可继续基于实机可读性微调最近结果摘要来源。
   - 仍不建议上屏完整 prompt、完整命令输出、raw JSON、长路径、详细 token 分项或复杂多页菜单。

4. **横屏 layout mode 仍是实验方向**
...
1. **先小幅打磨上屏摘要**
   - 如果长时间观察稳定，可以根据实际可读性继续微调最近结果摘要来源。
